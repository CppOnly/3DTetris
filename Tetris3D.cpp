...
void Tetris3D::BuildGameData() {
	BlockFactory::BuildBlockFactory(m_device.Get(), m_commandList.Get(), m_cbvHeap.Get());
...
void Tetris3D::MoveDownCurrBlock() {
	if (!m_pCurrBlock->TryTranslate(0.0f, 0.0f, 1.0f)) {
		...
		m_audioManger->PlayDropAudio();
 		m_score += m_pGrid->CalcRemovedNumOfLayer() * (m_level + 1) * 10;
		...
		m_pCurrBlock->SetPosition(2.0f, 2.0f, 1.0f);
		m_pCurrBlock->SetQuaternion(0.0f, 0.0f, 0.0f);
...
void Tetris3D::WorkerThread(int _index) {
	...
	while (_index >= 0 && _index < THREADNUM) {
		...
		if (_index == 0) {
			...
			m_pGrid->OnRender(m_device.Get(), pCommandList, m_cbvHeap.Get(), m_pipelineState.Get());
			pCommandList->Close();
		}
		else if (_index == 1) {
			SetCommonPipelineState(pCommandList);
			m_pCurrBlock->OnRender(m_device.Get(), pCommandList, m_cbvHeap.Get(), m_pipelineState.Get());
			pCommandList->Close();
		}
		else if (_index == 2) {
			SetCommonPipelineState_UI(pCommandList);
			m_pNextBlock->OnRender(m_device.Get(), pCommandList, m_cbvHeap.Get(), m_pipelineState.Get());
			pCommandList->Close();
		}
